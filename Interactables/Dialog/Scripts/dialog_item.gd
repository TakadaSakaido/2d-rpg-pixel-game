...
func _ready() -> void:
	if Engine.is_editor_hint():
		...
		editor_selection = Engine.get_singleton( "EditorInterface" ).get_singleton
...
func _on_selection_changed() -> void:
	...
	if not sel.is_empty():
		if self == sel[ 0 ]:
			example_dialog = load("res://GUI/Dialog System/DialogSystem.tscn").instantiate() as DialogSystemNode
			if example_dialog == null:
				return
